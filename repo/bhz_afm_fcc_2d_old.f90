...
program bhz_afm
  ...
  real(8)              :: epsik,fmesh,n(Norb),eigen(Norb)
  integer              :: Nkx,Nk
  real(8)              :: ix,iy
  real(8)              :: kx,ky
  ...
  do ix=1,Nkx
     ...
     do iy=1,Nkx
        ...
        do i=1,L
           ...
           fgr(i,:,:)=fgr(i,:,:) + inverse_g0k_afm(w,Hksite)/dble(Nk)
           w = xi*wm(i)+xmu
           fg(i,:,:) =fg(i,:,:)  + inverse_g0k_afm(w,Hksite)/dble(Nk)
        enddo
        count=count+1
  ...
  do ix=1,100
     ...
     Hk(:,:) = hk_bhz_afm(mh,lambda,kx,ky)
     Hksite = iso2site(Hk)
     eigen = Eigk_afm(Hksite)
     write(10,"(I,16F25.12)")ik,(eigen(i),i=1,Norb)
  enddo
  ...
  do iorb=1,Norb
     n(iorb) = fft_get_density(fg(:,iorb,iorb),beta)
     !n(iorb) = -2.d0*sum(dimag(fgr(:,iorb,iorb))*fermi(wr(:),beta))*fmesh/pi
  enddo
  open(10,file="observables.dat")
  write(10,"(24F20.12)")(n(iorb),iorb=1,Norb),sum(n)
  close(10)
  write(*,"(A,16F14.9,F14.9)")"Occupations                   =",n,sum(n)
