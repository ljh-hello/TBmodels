...
c     PROGRAM  : BETHE 
c     TYPE     : function
c     PURPOSE  : get the Hilber transfom of a given "zeta" with Bethe DOS
c     VERSION  : 06-10-2008
c     COMMENTS : 
c +-------------------------------------------------------------------+
      FUNCTION gfbethe(w,zeta,d)
      complex*16 gfbethe,zeta,sqroot,d2,one,xi
      real*8 d,sq,w,sig
      one=(1.d0,0.d0)
      xu=(0.d0,1.d0)
      ...
      gfbethe=2.d0*one/(zeta+sig*sqroot)
      return
      end
c +-------------------------------------------------------------------+
c     PROGRAM  : FERMI
...
c +-------------------------------------------------------------------+
      ...
      REAL*8 fermi, x, mu1, beta1
      fermi = 1.d0/(1.d0+exp(beta1*(x-mu1)))      
      return
      END
