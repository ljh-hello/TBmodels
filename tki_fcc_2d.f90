  ...
  program tki
    ...
    integer                                 :: Nk,Nktot,Nkpath,Nkx,Nky,Nkz
    ...
    allocate(kxgrid(Nkx),kygrid(Nky))
    ...
    Hk = build_hk_model(Nktot,Norb,hk_model,kxgrid,kygrid,[0d0])
    call write_hk_w90(trim(file),Norb,2,1,Hk,kxgrid,kygrid,[0d0])
  ...
  contains
    ...
    function hk_model(kpoint,N) result(hk)
      ...
      Hk(1:2,3:4) = 2d0*lambda*(sx*taux + sy*tauy)
      !Hk(1:2,3:4) =-2d0*lambda*(sx*transpose(conjg(taux)) + sy*transpose(conjg(tauy)))
      Hk(3:4,1:2) = 2d0*lambda*(sx*taux + sy*tauy)
